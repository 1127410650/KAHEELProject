  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[10px] font-semibold text-white/60">معاينة واجهة المتجر</p>
        <h2 className="truncate text-base font-black sm:text-lg">{template.title}</h2>
      </div>
      <span className="rounded-full bg-white px-3 py-1.5 text-[10px] font-black text-slate-900 sm:text-xs">{action}</span>
    </div>
  );
}
    ...
    <div className="mt-4 grid grid-cols-3 gap-2">
      {items.map((item, index) => (
        <div key={item} className="rounded-xl border border-white/15 bg-white/10 p-2.5 backdrop-blur-sm">
          ...
          <p className="mt-5 line-clamp-2 text-[9px] font-bold leading-4 sm:text-[11px]">{item}</p>
          <p className="mt-1 text-[8px] text-white/60 sm:text-[9px]">منتج تجريبي</p>
        </div>
      ))}
    </div>
  );
      ...
      <div className="mt-4 rounded-2xl border border-white/15 bg-white/10 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-[10px] leading-5 text-white/70">واجهة عصرية تركز على الصور، العروض، والتشكيلات المختارة.</p>
          </div>
        </div>
      </div>
    ...
    <Shell template={template}>
      ...
      <div className="mt-4 grid grid-cols-[1.3fr_0.7fr] gap-2">
        <div className="rounded-2xl border border-cyan-300/30 bg-black/25 p-4 shadow-[0_0_24px_rgba(34,211,238,0.18)]">
          ...
          <p className="mt-1 text-[10px] text-cyan-50/70">أجهزة، ألعاب، بطاقات وعروض حصرية.</p>
        </div>
        <div className="rounded-2xl border border-fuchsia-300/30 bg-fuchsia-500/10 p-3">
          ...
          <p className="mt-6 text-[10px] font-black">الأكثر رواجًا</p>
        </div>
      </div>
    ...
    <Shell template={template}>
      ...
      <div className="mt-4 flex items-center gap-3 rounded-2xl border border-lime-300/20 bg-black/20 p-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-[10px] text-white/70">اشتراكات، مدربون، حصص ومتابعة رياضية.</p>
        </div>
      ...
      <div className="mt-4 rounded-2xl border border-amber-200/20 bg-black/15 p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-[10px] text-white/70">أطباق مميزة، عروض عائلية وتجربة طلب واضحة.</p>
          </div>
        </div>
      </div>
  ...
  return (
    <Shell template={template}>
      ...
      <div className="mt-4 rounded-2xl border border-white/15 bg-white/10 p-4">
        ...
        <p className="mt-1 text-[10px] leading-5 text-white/70">تعرض أهم المنتجات والخدمات والتصنيفات بطريقة واضحة ومتجاوبة.</p>
      </div>
