...
export function PageSearchBar({ placeholder, href = "/search" }: { placeholder?: string | undefined; href?: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1 truncate text-desc font-semibold text-muted-foreground">
...
export function PageNoticeBar({
  ...
  return (
    <div
      role="status"
      className={`flex min-h-[34px] items-center gap-2 rounded-xl px-3 py-1 text-desc font-bold ${
        tone === "warn"
      ...
      {items.map((tile) => (
        <li key={tile.key} className="min-w-0">
          ...
          >
            ...
            <span className="w-full truncate text-desc font-bold text-brand-950">{tile.label}</span>
          </a>
        </li>
      ))}
    </ul>
  ...
  return (
    ...
    >
      <span className="min-w-0 flex-1">
        ...
        >
          <span
            className="block h-full rounded-full bg-[linear-gradient(90deg,var(--kt-primary-deep),var(--kt-primary))]"
            style={{ width: `${percent}%` }}
          />
        </span>
        <span className="num mt-1 block text-desc font-bold text-brand-800">
