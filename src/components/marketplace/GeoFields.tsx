...
/**
 * The country is a property of the account, not of the record being edited: it
 * is shown as a static line ("Country: Saudi Arabia") and only the city — always
 * scoped to that country — is selectable. Changing the country happens in
 * Settings → Account → Country.
 */
export function AccountCitySelect({
  cityId,
  onChange,
  required = false,
  cityLabel,
  ...
  const countryId = country.data?.id ?? null;
  const [term, setTerm] = useState("");
  const [suggesting, setSuggesting] = useState(false);
  const [suggestion, setSuggestion] = useState("");
  const [busy, setBusy] = useState(false);
  ...
  // Keep the parent's country aligned with the account country at all times.
  useEffect(() => {
    if (countryId) onChange({ countryId, cityId });
      ...
      {showCountry && (
        <div className="min-w-0 space-y-1.5">
          ...
          <p className="flex h-10 items-center text-sm text-muted-foreground">
            {country.data ? geoName(country.data, locale) : "—"}
          </p>
        </div>
      )}
