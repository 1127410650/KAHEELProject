...
export function FeaturedGuideStrip() {
  ...
  if (featured.isLoading) {
    return <div className="h-[104px] animate-pulse rounded-[var(--r-card)] bg-muted/40" aria-hidden />;
  }
  ...
  return (
    <section
      ...
      className="rounded-[var(--r-card)] border border-market-purple/35 bg-market-purple/[0.05] p-[var(--sp-3)] shadow-[0_0_24px_-12px_hsl(var(--primary)/0.55)]"
    >
      <h2 className="text-section mb-2 flex items-center gap-[var(--sp-2)] px-1 font-black text-market-purple">
      ...
      <div className="flex snap-x gap-2 overflow-x-auto pb-0.5">
        {rows.map((row) => (
          <Link
            ...
            className="flex h-[64px] w-[62%] shrink-0 snap-start flex-col justify-center gap-0.5 rounded-[var(--r-card)] border border-market-purple/25 bg-card px-3 transition active:scale-[0.97] sm:w-[30%]"
          >
            <span className="truncate text-desc font-black leading-5">{row.name_ar}</span>
            <span className="flex items-center gap-[var(--sp-2)] truncate text-desc font-bold text-muted-foreground">
