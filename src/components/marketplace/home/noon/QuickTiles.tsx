    ...
    >
      {TILES.map(({ key, slot, href, icon: Icon, ar: labelAr, en, tint }) => (
        <a
          ...
          className={`${TILE_SPAN[slotTileSize(slots.data, slot)]} k-press flex min-h-[76px] min-w-0 flex-col items-center justify-center gap-[var(--sp-2)] rounded-[var(--r-card)] px-[var(--sp-2)] py-2 text-center ring-1 outline-none transition hover:-translate-y-0.5 focus-visible:ring-2 focus-visible:ring-primary/45 ${tint}`}
        >
