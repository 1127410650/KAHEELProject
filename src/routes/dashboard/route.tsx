...
/**
 * Single gate for the whole account area (`/dashboard/*`).
 *
 * Until now every page repeated the sign-in check inside `DashboardShell`, which
 * runs only after the page module has already loaded and rendered. One parent
 * layout means a direct link to any account URL is resolved before its module is
...
export const Route = createFileRoute("/dashboard")({
  ...
  beforeLoad: async ({ location }) => {
    ...
    // `/dashboard` itself is not a page: the right landing screen depends on the
    // caller (admin console / work account / personal account), and `/me`
    // resolves that in one place.
    const path = location.pathname.replace(/\/+$/, "");
    if (path === "/dashboard") throw redirect({ to: "/me", replace: true });
  },
