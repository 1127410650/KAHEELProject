...
export const Route = createFileRoute("/admin/search")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "بحث موحّد داخل لوحة الإدارة عن المستخدمين والمنشآت والإعلانات والبلاغات وطلبات التوثيق.",
      },
      { property: "og:title", content: "البحث الإداري — إدارة المنصة" },
      { property: "og:description", content: "بحث موحّد داخل لوحة إدارة منصة تحقّق." },
            ...
            <section key={group.type} className="rounded-xl border border-border bg-card p-4">
              ...
              <div className="mt-3 grid gap-2">
                {group.items.map((item) => (
                  <Link
                    key={item.id}
                    href={searchHref(group.type, item.id)}
