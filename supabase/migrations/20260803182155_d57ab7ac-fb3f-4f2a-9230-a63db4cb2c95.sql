...
BEGIN
  ...
  RETURN QUERY
  WITH matched AS (
    SELECT r.id AS rid, count(*) OVER () AS total
...
BEGIN
  ...
  RETURN QUERY
  SELECT s.uid,
         coalesce(nullif(btrim(coalesce(p.full_name, '')), ''),
                  'مشرف #' || substr(replace(s.uid::text, '-', ''), 1, 6))
    FROM (
      SELECT pa.user_id AS uid FROM public.mkt_platform_admins pa
      UNION
