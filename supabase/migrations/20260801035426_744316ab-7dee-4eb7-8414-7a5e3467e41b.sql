CREATE OR REPLACE FUNCTION public.document_analysis_apply(
  _analysis_id uuid, _only_field_keys text[] DEFAULT NULL
...
BEGIN
  ...
  -- land: update in place (create the row if the project has none yet)
  ...
  -- deed: always a NEW version; previous row is preserved and marked superseded
  IF deed_set <> '{}'::jsonb THEN
    ...
    SELECT a.project_id, COALESCE(p.doc_type, 'deed'), COALESCE(p.version, 0) + 1, true, p.id, auth.uid(),
      ...
      'من تحليل المستند ' || a.document_id::text
    ...
    IF new_deed IS NULL THEN
      ...
      VALUES (a.project_id, 'deed', 1, true, auth.uid(),
        ...
        deed_set->>'owner_registry_no', 'من تحليل المستند ' || a.document_id::text)
      RETURNING id INTO new_deed;
    ELSE
  ...
  -- license: new row, previous rows kept untouched
  IF lic_set <> '{}'::jsonb THEN
    ...
    VALUES (a.project_id, auth.uid(), lic_set->>'license_no', lic_set->>'license_type',
      ...
      (lic_set->>'payment_date')::date, 'من تحليل المستند ' || a.document_id::text)
