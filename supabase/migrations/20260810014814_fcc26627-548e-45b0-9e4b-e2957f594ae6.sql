...
REVOKE ALL ON FUNCTION public.resolve_login_candidates(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.resolve_login_candidates(text) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.resolve_login_candidates(text) TO service_role;

-- توحيد التخزين: الأرقام المحلية تُحوَّل إلى الصيغة الدولية (+966) دون لمس أي بيانات أخرى
UPDATE public.profiles
SET phone = '+966' || regexp_replace(phone, '^0+', '')
WHERE phone IS NOT NULL
  AND phone NOT LIKE '+%'
  AND regexp_replace(phone, '\D', '', 'g') ~ '^0?5[0-9]{8}$';