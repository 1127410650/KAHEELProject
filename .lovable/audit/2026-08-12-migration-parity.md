...
# ملحق — فروق الصلاحيات التفصيلية (12/08/2026)

انحراف ACL الموصوف في البند 7-3 أعلاه صار موثَّقًا كائنًا-كائنًا في
`.lovable/audit/2026-08-12-acl-diff.md`: 429 دالة `mkt*` يمنحها الإنتاج لـ `anon`، منها
**40 استثناءً** مبرَّرًا بتتبع مستدعٍ فعلي (مسارات عامة + دوال داخل سياسات RLS تسري على anon)،
و**389** يُقترح سحبها (اتجاه REVOKE فقط) مع SQL واختبار انحدار وخطة تراجع لكل كائن.
لم تُنفَّذ أي كتابة على الإنتاج؛ التنفيذ موقوف على موافقة المالك الصريحة.
