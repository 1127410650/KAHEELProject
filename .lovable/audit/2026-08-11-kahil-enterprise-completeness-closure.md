...
| 27 | الأمان والصلاحيات وسجل العمليات | ✔ | `/admin/access-control` (يحوّل إليه `/admin/roles`), `/admin/ops-log`, `/admin/audit-log`, `/admin/staff.*` | `mkt_permission_catalog` (69 مفتاحًا، المصدر الموحّد)، `mkt_staff_permissions`، `mkt_platform_admins`، `mkt_ops_log`، `audit_log` | ✔ | ✔ | مُثبت بالتنفيذ الحيّ: مصفوفة 15 سيناريو، منع افتراضي لـ `anon`، RLS بلا سياسات كتابة، أثر تدقيقي كامل — التفاصيل في `.lovable/audit/2026-08-11-access-control-hardening.md`. المتبقي: مراجعة دورية مجدولة للصلاحيات |
