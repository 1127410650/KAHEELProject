...
| 27 | الأمان والصلاحيات وسجل العمليات | ✔ | `/admin/access-control` (يحوّل إليه `/admin/roles`), `/admin/ops-log`, `/admin/audit-log`, `/admin/staff.*` | `mkt_permission_catalog` (69 مفتاحًا، المصدر الموحّد)، `mkt_staff_permissions`، `mkt_platform_admins`، `mkt_ops_log`، `audit_log` | ✔ | ✔ | مُثبت بعد آخر هجرة بجلسات JWT حقيقية: رفض الدوال الإدارية التسع للمستخدم غير المخوّل مع before=after، حراسة الذات وآخر مالك، تزامن متسلسل، وصفوف نجاح تدقيقية، وانحدار المسارات العامة. رفض المعاملة لا يترك صف تطبيق لأن `RAISE` يتراجع عنه؛ موثق بصدق في `.lovable/audit/2026-08-11-access-control-hardening.md`. المتبقي: اعتماد المالك ومراجعة دورية مجدولة |
...
## تحديث 12/08/2026 — إغلاق «الأدوار والصلاحيات» (أمر مُقحَم بأولوية)

- صار `public.mkt_permission_catalog` المصدر الوحيد للصلاحيات: أي مفتاح خارجه يُرفض خادميًا (`Unknown permission key`).
- شاشة `/admin/access-control` مربوطة بالفرض الحقيقي؛ `/admin/roles` يحوّل إليها.
- **ثغرة حقيقية أُصلحت ولم تُطمس:** كان حفظ الصلاحيات يفشل كليًا (`42804` عند كتابة `entity_id` في `mkt_ops_log`) — أُصلح في الهجرة `20260812181646` وأُعيد اختبار الحفظ/السحب/التزامن بنجاح.
- تم إحكام المنع الافتراضي: `anon` لا يملك EXECUTE على أي دالة تكتب الأدوار أو الصلاحيات.
- **صدق بيئي:** الهجرات طُبِّقت على قاعدة الإنتاج `rgpnhzovtceitqxpiilf` (لا Staging)، فتغييرات الصلاحيات حيّة فورًا وإن لم تُنشر الواجهة.
- **إثبات حي لاحق:** أغلقت الهجرة `20260812183325` سلوك القائمة الفارغة في `mkt_content_health_open` إلى رفض `FORBIDDEN` صريح؛ اختُبرت الدوال التسع بمستخدم مسجّل بلا صلاحيات، واختُبر التزامن والمسارات العامة، دون بدء الدفعة 4 أو نشر الواجهة.
- الدفعة 4 لم تبدأ، ولم يُنشر شيء.
